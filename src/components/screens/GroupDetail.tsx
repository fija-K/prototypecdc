...
import { 
  ...
  TrendingUp, Award, Activity, FileText, Lock
} from 'lucide-react';
...
const INITIAL_GROUP_MENTORS: MentorItem[] = [
  {
    ...
    color: '#8B5CF6',
    students: [
      { id: 'ts1', name: 'Aarav Sharma', avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80', dept: 'CSE', year: '3rd Year', score: 892, growth: 18, status: 'High Potential', lastActive: '2 days ago' },
      { id: 'ts6', name: 'Neha Reddy', avatar: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=150&auto=format&fit=crop&q=80', dept: 'CSE', year: '3rd Year', score: 846, growth: 21, status: 'High Growth', lastActive: '1 day ago' },
      { id: 'ts3', name: 'Rohan Mehta', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80', dept: 'CSE', year: '3rd Year', score: 812, growth: 12, status: 'Strong', lastActive: '3 days ago' },
      { id: 'ts4', name: 'Ishita Verma', avatar: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?w=150&auto=format&fit=crop&q=80', dept: 'CSE', year: '3rd Year', score: 768, growth: 9, status: 'Developing', lastActive: '5 days ago' },
      { id: 'ts5', name: 'Karan Singh', avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&auto=format&fit=crop&q=80', dept: 'CSE', year: '3rd Year', score: 732, growth: 14, status: 'Strong', lastActive: '2 days ago' },
    ]
  },
  {
    id: 'gm2',
    ...
    color: '#2563EB',
    students: [
      { id: 'ts2', name: 'Riya Patel', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150&auto=format&fit=crop&q=80', dept: 'CSE', year: '3rd Year', score: 854, growth: 15, status: 'High Growth', lastActive: '12 hrs ago' },
      { id: 'ts3', name: 'Kabir Verma', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80', dept: 'CSE', year: '3rd Year', score: 792, growth: 11, status: 'Strong', lastActive: '1 day ago' },
    ]
  },
  {
    id: 'gm3',
    ...
    color: '#10B981',
    students: [
      { id: 'ts4', name: 'Sara Khan', avatar: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?w=150&auto=format&fit=crop&q=80', dept: 'CSE', year: '3rd Year', score: 748, growth: 10, status: 'Developing', lastActive: '3 days ago' },
      { id: 'ts5', name: 'Aditya Singh', avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&auto=format&fit=crop&q=80', dept: 'ECE', year: '3rd Year', score: 710, growth: 8, status: 'Strong', lastActive: '4 days ago' },
    ]
  }
];

export const GroupDetail: React.FC = () => {
  const { groups, selectedGroupId, setCurrentScreen, navigateToStudent, mentors } = useApp();

  // Find currently selected group or default to first group (Microsoft SDE Candidates)
  ...
  // Calculated Stats
  const totalStudents = currentGroup.studentCount || groupMentors.reduce((acc, m) => acc + m.assignedCount, 0); // 63
  const activeMentorsCount = groupMentors.length; // 3
  const avgStudentsPerMentor = Math.round(totalStudents / Math.max(1, activeMentorsCount)); // 21
    ...
    <div className="p-6 space-y-4 bg-[#F8FAFC] min-h-screen font-sans text-slate-800 select-none pb-20">
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2 text-xs font-semibold">
          ...
          <span className="text-blue-600 font-bold">{currentGroup.name}</span>
        </div>

        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pt-1">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-slate-900 tracking-tight">{currentGroup.name}</h1>
              <span className={`border font-semibold px-2.5 py-0.5 rounded text-xs ${
                currentGroup.statusBadge === 'Intervention' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                currentGroup.statusBadge === 'Draft' ? 'bg-slate-100 text-slate-600 border-slate-200' :
                'bg-emerald-50 text-emerald-700 border-emerald-200'
              }`}>
                {currentGroup.statusBadge || 'Active'}
              </span>
            </div>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              {currentGroup.subtitle || `${currentGroup.purpose} Group • ${totalStudents} Students • ${currentGroup.department || 'CSE'} • ${currentGroup.year || '3rd Year (2027)'}`}
            </p>
            <p className="text-[11px] text-slate-400 font-mono mt-0.5">
              Created on {currentGroup.createdAt || 'Aug 20, 2024'} <span className="text-slate-300">•</span> Owner: {currentGroup.owner} <span className="text-slate-300">•</span> Last updated: {currentGroup.updatedAt}
            </p>
          </div>
      ...
      {/* 2. Horizontal Group Navigation Tabs */}
      <div className="border-b border-slate-200 flex items-center gap-6 text-xs font-semibold select-none pt-1">
        ...
        <button 
          ...
          className={`py-2 px-1 border-b-2 transition-colors ${activeTab === 'students' ? 'text-blue-600 border-blue-600 font-bold' : 'text-slate-600 hover:text-slate-900 border-transparent'}`}
        >
          Students ({totalStudents})
        </button>
        <button 
          onClick={() => setActiveTab('mentors')} 
          className={`py-2 px-1 border-b-2 transition-colors ${activeTab === 'mentors' ? 'text-blue-600 border-blue-600 font-bold' : 'text-slate-600 hover:text-slate-900 border-transparent'}`}
        >
          Mentors ({activeMentorsCount})
        </button>
        <button 
          onClick={() => setActiveTab('intelligence')} 
          className={`py-2 px-1 border-b-2 transition-colors ${activeTab === 'intelligence' ? 'text-blue-600 border-blue-600 font-bold' : 'text-slate-600 hover:text-slate-900 border-transparent'}`}
        >
          Intelligence
        </button>
        <button 
          onClick={() => setActiveTab('interventions')} 
          className={`py-2 px-1 border-b-2 transition-colors ${activeTab === 'interventions' ? 'text-blue-600 border-blue-600 font-bold' : 'text-slate-600 hover:text-slate-900 border-transparent'}`}
        >
          Interventions
        </button>
        <button 
      ...
      {/* A. MENTORS TAB (REFERENCE SCREENSHOT REPLICA) */}
      {activeTab === 'mentors' && (
      ...
      {activeTab === 'students' && (
        <div className="bg-white border border-slate-200/90 rounded-xl p-4 shadow-xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              <h3 className="text-sm font-bold text-slate-900">Group Students Population ({totalStudents})</h3>
              <p className="text-xs text-slate-500">All registered student candidates belonging to {currentGroup.name}</p>
            </div>
            <div className="flex items-center gap-2">
              <input
                type="text"
                placeholder="Search candidates in group..."
                className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-xs text-slate-800 focus:outline-none w-64"
              />
              <button className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-semibold shadow-xs">
                Filter
              </button>
            </div>
          </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-200 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                ...
                <th className="pb-2">Dept / Year</th>
                <th className="pb-2 text-right">Tech Score</th>
                <th className="pb-2 text-right">Growth</th>
                <th className="pb-2">Assigned Mentor</th>
                <th className="pb-2">Status</th>
            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {[
                { id: 'ts1', name: 'Aarav Sharma', dept: 'CSE', year: '3rd Year', score: 892, growth: 18, mentor: 'Dr. Rahul Mehta', status: 'High Potential' },
                { id: 'ts2', name: 'Riya Patel', dept: 'CSE', year: '3rd Year', score: 854, growth: 15, mentor: 'Prof. Neha Sharma', status: 'High Growth' },
                { id: 'ts6', name: 'Neha Reddy', dept: 'CSE', year: '3rd Year', score: 846, growth: 21, mentor: 'Dr. Rahul Mehta', status: 'High Growth' },
                { id: 'ts3', name: 'Rohan Mehta', dept: 'CSE', year: '3rd Year', score: 812, growth: 12, mentor: 'Dr. Rahul Mehta', status: 'Strong' },
                { id: 'ts4', name: 'Ishita Verma', dept: 'CSE', year: '3rd Year', score: 768, growth: 9, mentor: 'Prof. Arjun Rao', status: 'Developing' },
                { id: 'ts5', name: 'Karan Singh', dept: 'CSE', year: '3rd Year', score: 732, growth: 14, mentor: 'Prof. Arjun Rao', status: 'Strong' }
              ].map(s => (
                <tr key={s.id} onClick={() => navigateToStudent(s.id)} className="hover:bg-slate-50 cursor-pointer">
                  <td className="py-2.5 font-bold text-slate-900">{s.name}</td>
                  <td className="py-2.5 text-slate-600">{s.dept} • {s.year}</td>
                  <td className="py-2.5 text-right font-mono font-bold text-slate-900">{s.score}</td>
                  <td className="py-2.5 text-right font-mono font-bold text-emerald-600">+{s.growth}%</td>
                  <td className="py-2.5 text-slate-700">{s.mentor}</td>
                  <td className="py-2.5">
                  ...
                  <td className="py-2.5 text-right">
                    <button onClick={e => { e.stopPropagation(); navigateToStudent(s.id); }} className="p-1 text-slate-400 hover:text-blue-600">
                      <Eye className="w-4 h-4" />
                    </button>
                  </td>
                </tr>
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-12 gap-4 text-xs font-sans">
          <div className="col-span-8 bg-white border border-slate-200/90 rounded-xl p-5 shadow-xs space-y-4">
            <h3 className="font-bold text-slate-900 text-sm border-b border-slate-100 pb-2">Group Health & Overview Summary</h3>
            <p className="text-slate-600 leading-relaxed">
              This cohort consists of {totalStudents} top technical candidates targeted for high-tier placement opportunities.
              The average technical score is <strong>{currentGroup.avgScore || 742} pts</strong> with a 30-day average growth of <strong className="text-emerald-600">+{currentGroup.growthRate || 12}%</strong>.
            </p>
            <div className="grid grid-cols-3 gap-3 pt-2">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                <span className="text-[10px] text-slate-400 uppercase font-semibold block">Active Problem Solvers</span>
                <span className="text-lg font-bold text-slate-900 font-mono">48 / {totalStudents}</span>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                <span className="text-[10px] text-slate-400 uppercase font-semibold block">Readiness Index</span>
                <span className="text-lg font-bold text-blue-600 font-mono">84.2%</span>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                <span className="text-[10px] text-slate-400 uppercase font-semibold block">Mentor Coverage</span>
                <span className="text-lg font-bold text-emerald-600 font-mono">100% Assigned</span>
              </div>
            </div>
          </div>

          <div className="col-span-4 bg-white border border-slate-200/90 rounded-xl p-5 shadow-xs space-y-3">
            <h3 className="font-bold text-slate-900 text-xs border-b border-slate-100 pb-2">Target Placement Roles</h3>
            <div className="space-y-2">
              <div className="p-2 bg-blue-50/70 border border-blue-100 rounded-lg flex items-center justify-between">
                <span className="font-bold text-slate-800">Microsoft SDE Intern</span>
                <span className="font-mono font-bold text-blue-600">92% Match</span>
              </div>
              <div className="p-2 bg-slate-50 border border-slate-100 rounded-lg flex items-center justify-between">
                <span className="font-bold text-slate-800">Amazon SDE I</span>
                <span className="font-mono font-bold text-slate-700">88% Match</span>
              </div>
              <div className="p-2 bg-slate-50 border border-slate-100 rounded-lg flex items-center justify-between">
                <span className="font-bold text-slate-800">Google STEP / SDE</span>
                <span className="font-mono font-bold text-slate-700">84% Match</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'intelligence' && (
        <div className="bg-white border border-slate-200/90 rounded-xl p-5 shadow-xs space-y-4 text-xs font-sans">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-bold text-slate-900 text-sm">Group Technical Intelligence & Skill Breakdown</h3>
              <p className="text-slate-500 text-xs">Deep analytics across DSA, Development, System Design, and CP contest performance</p>
            </div>
          </div>

          <div className="grid grid-cols-4 gap-4 pt-1">
            <div className="p-3 bg-purple-50/60 border border-purple-100 rounded-xl">
              <span className="text-[10px] text-purple-700 font-bold uppercase block">DSA Mastery</span>
              <span className="text-xl font-bold text-purple-900 font-mono">82.4 %</span>
              <span className="text-[10px] text-purple-600 block mt-1">Avg 340+ LeetCode Medium/Hard</span>
            </div>
            <div className="p-3 bg-blue-50/60 border border-blue-100 rounded-xl">
              <span className="text-[10px] text-blue-700 font-bold uppercase block">Dev Velocity</span>
              <span className="text-xl font-bold text-blue-900 font-mono">74.6 %</span>
              <span className="text-[10px] text-blue-600 block mt-1">Avg 18 GitHub commits/month</span>
            </div>
            <div className="p-3 bg-emerald-50/60 border border-emerald-100 rounded-xl">
              <span className="text-[10px] text-emerald-700 font-bold uppercase block">CP Rating</span>
              <span className="text-xl font-bold text-emerald-900 font-mono">1540 pts</span>
              <span className="text-[10px] text-emerald-600 block mt-1">Codeforces Specialist avg</span>
            </div>
            <div className="p-3 bg-amber-50/60 border border-amber-100 rounded-xl">
              <span className="text-[10px] text-amber-700 font-bold uppercase block">Consistency Score</span>
              <span className="text-xl font-bold text-amber-900 font-mono">88.5 %</span>
              <span className="text-[10px] text-amber-600 block mt-1">Active 5+ days per week</span>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'interventions' && (
        <div className="bg-white border border-slate-200/90 rounded-xl p-5 shadow-xs space-y-4 text-xs font-sans">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-bold text-slate-900 text-sm">Group Active Interventions</h3>
              <p className="text-slate-500 text-xs">Targeted training and remediation programs assigned to this cohort</p>
            </div>
          </div>

          <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-slate-900 text-sm">Microsoft Mock Technical Interview Drill</span>
              <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded font-semibold text-[10px]">Active</span>
            </div>
            <p className="text-slate-600 text-xs">Focus on Graph Algorithms, DP optimization, and System Design whiteboarding.</p>
            <div className="pt-2 flex items-center justify-between text-[11px] text-slate-500 font-mono">
              <span>Mentor: Dr. Rahul Mehta</span>
              <span>Deadline: Sep 20, 2024</span>
              <span className="text-blue-600 font-bold">Progress: 65%</span>
            </div>
          </div>
        </div>
      )}
      ...
      {/* F. ACTIVITY LOG TAB */}
      {activeTab === 'activity' && (
        <div className="bg-white border border-slate-200/90 rounded-xl p-5 shadow-xs space-y-3 text-xs font-sans">
          <h3 className="font-bold text-slate-900 text-sm border-b border-slate-100 pb-2">Group Audit & Activity History</h3>
          <div className="space-y-3 pt-1">
            <div className="flex items-start gap-3 text-xs">
              <div className="w-2 h-2 rounded-full bg-blue-500 mt-1.5 shrink-0"></div>
              <div>
                <span className="font-bold text-slate-900">Dr. Rahul Mehta</span> assigned 5 new students to Microsoft SDE Candidates
                <span className="text-[10px] text-slate-400 block font-mono">2 hours ago</span>
              </div>
            </div>
            <div className="flex items-start gap-3 text-xs">
              <div className="w-2 h-2 rounded-full bg-emerald-500 mt-1.5 shrink-0"></div>
              <div>
                <span className="font-bold text-slate-900">Placement Cell (CTC)</span> created group Microsoft SDE Candidates
                <span className="text-[10px] text-slate-400 block font-mono">Aug 20, 2024</span>
              </div>
            </div>
          </div>
        </div>
